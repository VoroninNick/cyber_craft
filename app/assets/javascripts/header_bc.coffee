...
wheel_last_time = false
$(window).on "wheel", (e)->

  date_now = Date.now()
  if wheel_last_time == false
    diff = false
  else
    diff = date_now - wheel_last_time


  wheel_timeout = 50
  ignored = wheel_last_time != false && diff <= wheel_timeout

  if ignored
    console.log "ignored: diff: ", diff
  else
